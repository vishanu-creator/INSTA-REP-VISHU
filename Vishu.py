...
def nx():
    os.system("clear")
    Banner = f"""{B}{E}=============================={B}
|{F}[+] YouTube    : {B}| Mr.Indian Tricker
|{F}[+] TeleGram   : {B} VISHANU XD
|{F}[+] Instagram  : {B} Vishanu_0x1
...
user = input(f' {F}({M}1{F}) {M} Enter The Target Username IG{F} :  ' + O)
print(X + ' ═════════════════════════════════  ')
ses = input(f' {F}({M}2{F}) {M} Enter Your SessionId IG{F} :  ' + O)
csr = token_hex(8) * 2
console = Console()
bb = 0
gg = 0
...
while True:
  ...
  if '"status":"ok"' in res:
      ...
      table.add_row("Dev", "VISHANU~~ @vishanu_0x1")
      console.print(table)
  else:
      bb += 1
      os.system('clear')
